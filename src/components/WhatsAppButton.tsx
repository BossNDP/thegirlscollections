...
export default function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-[80] flex items-center group">
      {/* Desktop Hover Label Badge */}
      <span className="hidden sm:inline-block opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0 text-[11px] font-sans font-semibold uppercase tracking-[0.1em] text-navy bg-white/95 backdrop-blur-md px-3.5 py-1.5 rounded-full shadow-lg border border-navy/10 mr-2 pointer-events-none select-none">
        Chat on WhatsApp
      </span>

      {/* Luxury WhatsApp Floating Action Button */}
      <a
        ...
        className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-[#25D366] to-[#128C7E] text-white shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center border-2 border-white/90 ring-1 ring-black/10"
        aria-label="Contact via WhatsApp"
        title="Chat with us on WhatsApp"
      >
        {/* Subtle Glow Ring */}
        <span
          className="absolute -inset-1 rounded-full bg-[#25D366]/35 animate-ping opacity-65 pointer-events-none"
          style={{ animationDuration: '3.5s' }}
        />

        {/* Crisp Official WhatsApp SVG Icon */}
        <svg
          className="w-6 h-6 sm:w-7 sm:h-7 fill-current text-white relative z-10 drop-shadow-xs"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
