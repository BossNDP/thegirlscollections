  ...
  return (
    ...
    >
      ...
      <div className="relative p-[3px] rounded-full border border-zariGold/40 group-hover:border-zariGold group-hover:shadow-[0_8px_24px_rgba(180,134,60,0.3)] transition-all bg-ivory shadow-sm shrink-0">
        ...
        {/* INNER CREATIVE LIGHT BABY PINK CIRCLE WITH ROSE GOLD SATIN GLOW */}
        ...
        {/* SINGLE POPPING MODEL IMAGE OVERLAY */}
        {category.image && (
          <div
...
const ShopMoreCircle: React.FC<ShopMoreCircleProps> = ({ total }) => {
  ...
  return (
    <button
      ...
      className="group relative flex flex-col items-center shrink-0 snap-start select-none outline-none focus-visible:ring-2 focus-visible:ring-zariGold rounded-full transition-all duration-300 active:scale-[0.95] hover:scale-[1.03] cursor-pointer pt-3 min-h-[44px]"
...
interface CircularCategoryScrollerProps {
  filter?: 'all' | 'women' | 'kids';
}

export const CircularCategoryScroller: React.FC<CircularCategoryScrollerProps> = ({ filter = 'all' }) => {
  ...
  const displayedCategories = useMemo(() => {
    if (filter === 'women') return womenFeatured;
    if (filter === 'kids') return kidsFeatured;
    return [...womenFeatured, ...kidsFeatured];
  }, [filter, womenFeatured, kidsFeatured]);

  const totalCount = useMemo(() => {
    if (filter === 'women') return WOMEN_CATEGORIES.length;
    if (filter === 'kids') return KIDS_CATEGORIES.length;
    return WOMEN_CATEGORIES.length + KIDS_CATEGORIES.length;
  }, [filter]);
  ...
  return (
    <section className="w-full bg-ivory pt-6 pb-8 sm:pt-10 sm:pb-12 border-b border-zariGold/15 overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8">
        
        {/* CLEAN HEADER ROW (No duplicate toggle) */}
        <div className="flex items-end justify-between mb-6 border-b border-zariGold/15 pb-3">
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-[11px] sm:text-xs font-sans font-bold tracking-[0.2em] uppercase text-zariGold">
                WARDROBE UNIVERSE
              </span>
            </div>
            <h2 className="text-xl sm:text-3xl font-serif text-inkNavy font-bold tracking-tight leading-tight">
              {filter === 'women'
                ? "Women's Couture Categories"
                : filter === 'kids'
                ? 'Little Royalty Kids Categories'
        ...
        {/* PROMINENT CIRCULAR CATEGORY ROW */}
        <div
          ref={scrollRef}
